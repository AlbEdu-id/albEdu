...
const APP_CONFIG = {    
    ...
    // 🔥 UPDATED: Siswa hanya redirect ke ujian/index.html    
    REDIRECT_PATHS: {    
        admin: '/admin/index.html',    
        siswa: '/ujian/index.html', // ⚠️ DIUBAH: siswa ke ujian saja    
        login: '/login.html',    
        logout: '/login.html'    
    },    
...
// 🔥 UPDATED: Siswa hanya boleh akses ujian/index.html    
const ROLE_WHITELIST = {    
    ...
    siswa: [    
        APP_CONFIG.BASE_PATH + '/ujian/index.html', // ⚠️ DIUBAH: hanya ujian    
        APP_CONFIG.BASE_PATH + '/login.html'    
    ]    
};    
...
function redirectBasedOnRole() {    
    ...
    window.Auth.redirectInProgress = true;    
    ...
    if (currentUrl === targetUrl || currentUrl === targetUrl + '/') {    
        ...
        window.Auth.redirectInProgress = false;    
        return;    
    }    
...
async function checkPageAccess() {      
    ...
    console.log(`🔒 Mengecek akses: ${currentPath} | Role: ${window.Auth?.userRole} | Base: ${getBasePath()}`);    
...
function showAccessDenied() {      
    ...
    const base = getBasePath();    
    // 🔥 UPDATED: Redirect siswa kembali ke ujian    
    const redirectPath = window.Auth?.userRole === 'siswa'   
        ? buildFullPath('/ujian/index.html')  // Siswa kembali ke ujian  
        : buildFullPath('/admin/index.html'); // Admin kembali ke admin  
    ...
    document.body.innerHTML = `    
        ...
        ">    
            ...
            <p style="font-size: 16px; max-width: 400px; color: #4b5563;">    
                ...
                Silakan kembali ke halaman yang diizinkan.    
            </p>    
            <p style="font-size: 14px; color: #6b7280; margin-top: 10px;">    
                Role: <strong>${window.Auth?.userRole || 'unknown'}</strong><br>    
                Base Path: <code>${base}</code><br>    
                Current Path: <code>${window.location.pathname}</code>    
            </p>    
            <a href="${redirectPath}" style="    
                margin-top: 20px;    
                padding: 10px 20px;    
                background: #2563eb;    
                color: white;    
...
window.ByteWard = {    
    ...
    debugByteWard: function() {    
        ...
        console.log('User Role:', window.Auth?.userRole);    
        console.log('User Data:', window.Auth?.userData);    
        console.log('Profile Complete:', window.UI?.profileState?.isProfileComplete);    
        console.log('Current Path:', window.location.pathname);    
        console.log('Is Login Page:', isLoginPage());    
        console.log('Within App Scope:', isWithinAppScope());    
        console.log('Auth Ready:', window.Auth?.authReady);    
        console.log('Redirect in Progress:', window.Auth?.redirectInProgress);    
        console.log('==========================');    
    }    
};    
